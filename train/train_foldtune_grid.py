...
using grid search on randomly held out training chunk.
...
# Only need if want to tune HSM/D. Not used for HSM/ID tuning.
HSMD_DOMS = ['Kinase_TK', 'SH3', 'PTB', 'PTP', 'PDZ', 'SH2', 'WH1', 'WW']
...
def create_parser():
    ...
    # Hyperparameters, including parameters of grid search
    hyperparam_group = parser.add_argument_group(title="Learning hyperparameters")
    hyperparam_group.add_argument("--min_log_lambda", type=float, default=-7, help="Lowest log learning rate to consider for grid search. Default: -7")
    hyperparam_group.add_argument("--max_log_lambda", type=float, default=-4, help="Highest log learning rate to consider for grid search. Default: -4")
    hyperparam_group.add_argument("--min_log_learning_rate", type=float, default=-5, help="Lowest log learning rate to consider for grid search. Default: -5")
    hyperparam_group.add_argument("--max_log_learning_rate", type=float, default=-3, help="Highest log learning rate to consider for grid search. Default: -3")
    hyperparam_group.add_argument("-e", "--epochs", type=int, default=60,
            help="Max number of epochs to run the strategy for. Default: 60")
    hyperparam_group.add_argument("-v", "--validate_step", type=int, default=5,
            help="The step number to run validation on). Default: 5.")
...
def hyperparam_grid_search(options):
   ...
   grid_lambda = np.logspace(options.min_log_lambda,options.max_log_lambda,num=int(options.max_log_lambda - options.min_log_lambda)+1)
   # HSM/D hyperparameter grid over all domain regularization penalties
   if options.hsm_d:
      ndomains = len(HSMD_DOMS)
      # full grid of num_lambdas^8
      lambda_mesh = np.meshgrid(*np.split(np.tile(grid_lambda, ndomains), ndomains))
      lambda_mesh = list(map(np.ravel, lambda_mesh))
      grid_lambda = []
      for ls in zip(*lambda_mesh):
         grid_lambda.append(' '.join([HSMD_DOMS[i]+':'+str(ls[i]) for i in range(ndomains)]))

   # full hyperparameter grid
   grid_lr = np.logspace(options.min_log_learning_rate,options.max_log_learning_rate,num=int(options.max_log_learning_rate - options.min_log_learning_rate) + 1)
   grid_epochs = np.arange(options.validate_step,options.epochs+options.validate_step,options.validate_step)
   mesh = np.meshgrid(grid_lr, grid_lambda)
   mesh = list(map(np.ravel, mesh))
   ...
   for i, (lr, lambdap) in enumerate(zip(*mesh)):
      ...
      aucs.extend(list(np.mean(auc, axis=1)))
   ...
   optimal = {constants.KEY_learning_rate: float(mesh[0][ndx]),
              constants.KEY_lambdas: optimal_lambdap,
              constants.KEY_epochs: int(mesh[2][ndx]),
              'auc': aucs[ndx]}
   fname = os.path.join(options.output_directory, 'optimal_hyperparameters.json')
   with open(fname, 'w+') as f:
      json.dump(optimal, f)
...
if __name__=='__main__':
    ...
    optimal_params = hyperparam_grid_search(options)

    # train with full training set
    arguments = format_arguments_directory(options, optimal_params[constants.KEY_learning_rate], optimal_params[constants.KEY_epochs])
    lambdap = optimal_params[constants.KEY_lambdas]
    if isinstance(lambdap, str):
